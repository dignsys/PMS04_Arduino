...
bool IRrecv::decodeMitsubishi112(decode_results *results, uint16_t offset,
  ...
  decode_type_t typeguess = decode_type_t::UNKNOWN_DEV;
  uint16_t hdrspace;
  uint16_t bitmark;
  uint16_t onespace;
  uint16_t zerospace;
  ...
  if (typeguess == decode_type_t::UNKNOWN_DEV &&  // We didn't match Mitsubishi112
  ...
  if (typeguess == decode_type_t::UNKNOWN_DEV) return false;  // No header matched.
  offset++;
